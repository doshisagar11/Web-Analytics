...
class DataFrame(object):
    ...
    def __init__(self, list_of_lists, header=True):
        ...
        if header: # then do this
            ...
            self.data_spaces = list_of_lists[1:]
            for item in self.data_spaces:
        ...
        # if what was passed into header is False or has a value that is equivalent to false, i.e. bool(header) is False
            ...
            self.data_spaces = list_of_lists
            for item in self.data_spaces:
                item = [x.strip(' ') for x in item]
                self.data.append(item)
    ...
    def sort_by(self, column_name, reverse):
        initial_index = self.header.index(column_name)

        print(initial_index)
        if (initial_index == 0 or initial_index == 8 or initial_index == 9):
            if reverse:
                return sorted(self.data, key=operator.itemgetter(3), reverse=True)


            else:
                return sorted(self.data, key=operator.itemgetter(3), reverse=False)

        elif initial_index == 2 or initial_index == 10 or initial_index == 11:
            self.data_temp=[]
            for val in self.data:
                temp = str(val[initial_index])
                temp = temp.replace(",", "")
                val[initial_index] = float(temp)
                self.data_temp.append(val)

            if reverse:
                return sorted(self.data_temp, key = operator.itemgetter(initial_index), reverse=True)
            else:
                return sorted(self.data_temp, key=operator.itemgetter(initial_index))

        else:
            return sorted(self.data, key=operator.itemgetter(initial_index))
